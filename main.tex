...
\begin{itemize}[leftmargin=*]
        ...
        \item[] Summensatz\tabto{2cm} $\sin(\alpha)\cdot\cos(\alpha)=\sin(2\cdot\alpha)$
\end{itemize}
...
\subsection{Reibung}
\begin{itemize}[leftmargin=*]
        \item[] Haftreibung\tabto{2cm} $F_{R_{max}}=F_N\cdot\mu$
...
\begin{itemize}[leftmargin=*]
        ...
        \item [] Kreis\tabto{2cm} $I_{x_\circ}=\frac{d^2\cdot\pi}{4}$
...
Abstandasabhängige Gesamt-Normalspannung:
\begin{itemize}[leftmargin=*]
        \item [] $\sigma(y_{HA},z_{HA})=\frac{M_{HA}}{I_1}\cdot z_{HA}+\frac{M_{NA}}{I_2}\cdot y_{HA}\pm\frac{F_{Z/D}}{A}$
        \item [] $\sigma_x(y,z)=\frac{M_y}{I_{yy}}\cdot z+\frac{M_x}{I_{zz}}\cdot y\pm\frac{F_{Z/D}}{A}$
\end{itemize}
...
\\<Bild>
\begin{itemize}[leftmargin=*]
        \item [] $F_K=\frac{\pi^2\cdot E\cdot I}{L_K^2}$\\
        ...
        \begin{math}
                ...
                \begin{cases}
                        \text{ ...Schlankheitsgrad}\\
                        >90\textsl{ bzw.} >\lambda_g\rightarrow\text{Euler}\\
                        \text{ansonsten}\rightarrow\text{Tetmajr oder Quetschen (Z/D)}\\
                \end{cases}
        \end{math}
\end{itemize}

Knickung nach Tetmajr:\\
\tabto{0.35cm}$\sigma_K=a-b\cdot\lambda+c\cdot\lambda^2$\tabto{3.5cm}$\text{a, b, c aus Tabelle}$
...
\subsubsection{Querkraftbiegung}
%[!]TODO: Bilder einfügen
\begin{itemize}[leftmargin=*]
        \item [] $\tau_S=\frac{F_Q\cdot s_x}{I_x\cdot b_{(z)}}$
...
\begin{itemize}[leftmargin=*]
        ...
        \item [] $\phi=\frac{M_T\cdot l}{I_p\cdot G}$
\end{itemize}
...
\tabto{0.35cm}$W''(x)=-\frac{M(x)}{E\cdot I}$\tabto{3cm}{\scriptsize Variablen/-anzahl und Gleichungen/-anzahl Dokumentieren}
...
%[!]TODO: Bild einfügen
\begin{itemize}[leftmargin=*]
        \item [] $y_{SD}=y_D-y_S=\frac{I_x^S}{A\cdot y_S}$
        \item [] $x_D=x_S$
\end{itemize}
...
\begin{itemize}[leftmargin=*]
        ...
        \item [] Druckform:\\
                $\rho\cdot g\cdot h_1+\rho\cdot\frac{v_1^2}{2}+p_1+\frac{P_\text{P}\cdot\rho}{\dot{m}}=\rho\cdot g\cdot h_2+\rho\cdot\frac{v_2^2}{2}+p_2+\Delta{p_v}$
        \item [] Höhenform:\\
                $h_1+\frac{v_1^2}{2\cdot g}+\frac{p_1}{\rho\cdot g}+\frac{P_\text{P}}{\dot{m}\cdot g}=h_2+\frac{v_2^2}{2\cdot g}+\frac{p_2}{\rho\cdot g}+\Delta{h_v}$
\end{itemize}

\subsection{Strömungsverluste \& Moody-Diagramm}
\begin{itemize}[leftmargin=*]
        \item [] örtliche Verluste:
        \item [] $h_\text{VE}=\zeta\cdot\frac{v^2}{2\cdot g}$
        \item [] Streckenverluste:
...
\begin{itemize}[leftmargin=*]
        \item [] $|S|=p_0\cdot A+m\cdot v$\tabto{3cm}$S$ ...Stützkraft
\end{itemize}
...
\begin{itemize}[leftmargin=*]
        \item [] Rechtsläufige Kresiprozesse:\\
        $\eta_{th}=\frac{|A|}{Q_{zu}}=\frac{q_{zu}-|q_{ab}|}{q_{zu}}\qquad\qquad A_{KP}=-(Q_{zu}-|Q_{ab}|)\qquad\qquad P=A\cdot\dot{m}$
        \item [] Isentrope Wirkungsgrade:\\
        $\eta_{is\text{V}}=\frac{h_{2s}-h_1}{h_2-h_1}\qquad\qquad \eta_{is\text{T}}=\frac{h_2-h_1}{h_{2s}-h_1}$
        \item [] Linksläufige Kresiprozesse:\\
        $\epsilon_{KM}=\frac{q_{zu}}{q_{ab}-q_{zu}}\qquad\qquad\epsilon_{WP}=\frac{q_{ab}}{q_{ab}-q_{zu}}$
        \item [] {\scriptsize Anmerkung: Dampfprozesse: kein $c_p\cdot\Delta{T}\quad\Rightarrow$ Tabelle}
\end{itemize}
...
\subsection{Dynamik}
\begin{itemize}[leftmargin=*]
        \item [] Impulssatz: \tabto{3cm}$m\cdot\ddot{x}=\sum{F}$
        \item [] Drallsatz: \tabto{3cm}$J\cdot\ddot{\phi}=\sum{M}$
        \item [] wichtige Kräfte: \tabto{3cm}$F_\text{Feder}=c\cdot x_\text{F}\qquad F_\text{Dämpfer}=k\cdot\dot{x_\text{D}}$
        \item [] wichtige Massenträgheitsmomente:\\$J_\text{Zylinder}^S=\frac{m\cdot r^2}{2}\qquad J_\text{Kugel}^S=\frac{2}{5}\cdot m\cdot r^2\qquad J_\text{Stab}^S=\frac{m\cdot l^2}{12}$
        \item [] Satz von Steiner: \tabto{3cm}$J^P=J^S+m\cdot r^2$
\end{itemize}
...
\begin{itemize}[leftmargin=*]
        ...
        \item [] potentielle Energie:\\$E_\text{pot}=W_\text{pot}=m\cdot g\cdot h$
        \item [] kinetische Energie:\\$E_\text{transl}=W_\text{transl}=\frac{m\cdot v^2}{2}\qquad E_\text{rot}=W_\text{rot}=\frac{J\cdot\omega^2}{2}$
        \item [] Federenergie:\\$E_\text{el}=W_\text{el}=\frac{c\cdot(s_1^2-s_2^2)}{2}=\frac{c\cdot(\phi_1^2-\phi_2^2)}{2}$
        \item [] Reibungsenergie:\\$E_\text{reib}=W_\text{reib}=F_N\cdot\mu\cdot s$
        \item [] Massenreduktion:\\$m_{red}\cdot\frac{v_{red}^2}{2}=J_{red}\cdot\frac{\omega_{red}^2}{2}=\sum{E_\text{kin}}$
        \item [] Leistung:\\$P=F\cdot v=M\cdot\omega=\frac{F\cdot s}{t}$
\end{itemize}

\subsection{Impuls \& Stoß}
\begin{itemize}[leftmargin=*]
        \item [] Impulserhaltung: \tabto{3cm}$m_1\cdot\vec{v_1}=m_2\cdot\vec{v_2}=\text{konst.}$
        \item [] Stoß:
        \subitem $u=\frac{m_1\cdot v_1+m_2\cdot v_2}{m_1+m_2}$
        \subitem $s_{K1}=m_1\cdot(u-v_1)\qquad s_{R1}=m_1\cdot(v_1'-u)$
        \subitem $s_{K2}=m_2\cdot(u-v_2)\qquad s_{R2}=m_2\cdot(v_2'-u)$
        \subitem $k=\frac{s_{R1}}{s_{K1}}=\frac{s_{R2}}{s_{K2}}=\frac{u-v_1'}{v_1-u}=\frac{v_2'-u}{u-v_2}$
        \subitem $E_\text{kin0}E_\text{pot0}-E_v=E_\text{kin1}E_\text{pot1}\qquad\qquad E_v$ ...Verformungsenergie
\end{itemize}

\subsection{Schwingungen}
\begin{itemize}[leftmargin=*]
        \item [] \textbf{freie ungedämpfte Schwingung}
        \subitem $\ddot{\variant{x}}+\omega^2\cdot\variant{x}=0\qquad\qquad m\cdot\ddot{\variant{x}}+c\cdot\variant{x}=0$
        \subitem $\variant{x}(t)=A\cdot\sin(\omega\cdot t)+B\cdot\cos(\omega\cdot t)\qquad$bzw.$\qquad\variant{x}(t)=C\cdot\sin(\omega\cdot t+\psi)$
        ...
        \item [] \textbf{freie gedämpfte Schwingung}
        \subitem $\ddot{\variant{x}}+2\delta\cdot\dot{\variant{x}}+\omega^2\cdot\variant{x}=0$\\ \\
        \subitem schwache Dämpfung:\tabto{3.5cm}$\delta^2<\omega^2$
        \subsubitem 
        \subsubitem $\variant{x}(t)=e^{-\delta\cdot t}\cdot A\cdot\sin(\omega_d\cdot t)+B\cdot\cos(\omega_d\cdot t)\qquad$
        \subsubitem \tabto{1.8cm}bzw.
        \subsubitem $\variant{x}(t)=C\cdot e^{-\delta\cdot t}\cdot\sin(\omega_d\cdot t+\psi)$
        \subsubitem $\quad\longrightarrow\quad\omega_d=\sqrt{\omega^2-\delta^2}$
        \subsubitem $\quad\longrightarrow\quad A$ \& $B\quad\Leftarrow\quad$ aus Randbedingungen ermitteln
        \subsubitem $\quad\longrightarrow\quad C=\sqrt{A^2+B^2}$
        \subsubitem $\quad\longrightarrow\quad\psi=\arctan\left(\frac{A}{B}\right)$\\ \\
        \subitem starke Dämpfung:\tabto{3.5cm}$\delta^2>\omega^2$
        \subsubitem $\variant{x}(t)=e^{-\delta\cdot t}\cdot\left(A\cdot e^{\sqrt{\delta^2-\omega^2}\cdot t}+B\cdot e^{\sqrt{\delta^2-\omega^2}\cdot t}\right)$\\ \\
        \subitem aperiodischer Grenzfall:\tabto{3.5cm}$\delta^2=\omega^2$
        \subsubitem $\variant{x}(t)=e^{-\delta\cdot t}\cdot(A+B\cdot t)$
        \item [] \textbf{erzwungene Schwingung}
        \subitem $\ddot{\variant{x}}+2\delta\cdot\dot{\variant{x}}+\omega^2\cdot\variant{x}=A_e\cdot\sin(\Omega\cdot t)$\tabto{5.26cm}oder$\quad\dots=A_e\cdot\cos(\Omega\cdot t)$
        \subitem \tabto{5.26cm}{\scriptsize wenn $\omega=\Omega\quad\rightarrow\quad$Resonanz}\\ \\
        \subitem $x_p(t)=C\cdot\sin(\Omega\cdot t+\psi)$\tabto{5.26cm}oder$\quad\dots=C\cdot\cos(\Omega\cdot t+\psi)$
        \subitem $\quad\longrightarrow\quad A=\frac{A_e}{\sqrt{(\omega^2-\Omega^2)^2+4\cdot\delta^2\cdot\Omega}}$
        \subitem $\quad\longrightarrow\quad\psi=\arctan(\frac{2\cdot\delta\cdot\Omega}{\omega^2-\Omega^2})\qquad$ für $\psi<0$: $\quad\psi_{neu}=\psi+\pi$
\end{itemize}
